# 🤝 Contributing to Awesome Omni Skills (Español)
...
>**Awesome Omni Skills es un repositorio público de habilidades, un repositorio curado de derivados de mejores prácticas mejorados y las superficies de tiempo de ejecución construidas sobre ese catálogo.**
...
| 🔁 Ingesta basada en repositorio | Las importaciones externas operadas por mantenedores todavía se encuentran bajo "habilidades/" y siguen el mismo estándar de revisión |
...
| 📥 Revise un PR de admisión basado en repositorio | `external-import/<source>` ramas dirigidas a `skills/` |
| 🌍 Proponer un nuevo repositorio ascendente | `REPOSITORIO-FUENTES.md` |
| 📖 Actualizar la guía para contribuyentes | `docs/colaboradores/` |
| 🖥️ Mejorar CLI, instalador o scripts | `paquetes/cli/` y `herramientas/scripts/` |
...
>**💡 Consejo:**Los paquetes de habilidades de nivel de lanzamiento deben incluir `agentes/`, `referencias/`, `ejemplos/` y `scripts/`. Pero la superficie de entrada es intencionalmente permisiva: se permite una habilidad entrante nativa mínima y la tubería del potenciador genera el derivado más fuerte.### 🔀 Two Native Intake Paths

El repositorio público acepta habilidades nativas a través de dos vías operativas:

| Camino | Como llega | Expectativa de revisión |
|:-----|:---------------|:-------------------|
| 👤 Relaciones públicas colaboradoras directas | Una rama humana edita `skills/` directamente | Validar la habilidad, los artefactos generados y el impacto del paquete |
| 📥 Entrada externa basada en repositorio | El PR `external-import/<source>` operado por el mantenedor importa habilidades de otro repositorio revisado | Validar la habilidad más la procedencia, la propiedad ascendente y las notas de licencia |

Ambas rutas llegan a `skills/`, ambas activan el mismo validador público y ambas pueden producir un PR complementario seleccionado `skills_omni/`.

Si desea proponer un repositorio ascendente en lugar de enviar una habilidad directamente, edite [REPOSITORY-SOURCES.md](REPOSITORY-SOURCES.md). La propuesta normal ahora es el repositorio primero:

- proporcionar `repo_url`
- Deje `branch` como `auto` a menos que necesite una rama no predeterminada
- deja `skills_path` como `auto` a menos que conozcas la raíz exacta

Fusionar todavía hay una puerta de revisión pública, no un activador de sincronización automática.### 🌐 Language Policy
...
>**⚠️ Importante:**La validación es el contrato utilizado por CLI, API, MCP, A2A, manifiestos, archivos y automatización de versiones. Trate los metadatos generados como parte de la superficie de revisión, no como resultados desechables.

Si toca la marca, los metadatos del paquete, las URL del repositorio o la copia del encabezado del repositorio, ejecute también:```bash
npm run repo:metadata:print
```

Ese comando imprime la descripción de GitHub, la página de inicio y el contrato de temas controlados por el código fuente derivados de `data/project_identity.json`.### 📥 Intake Policy
...
Si toca `packages/cli/`, `packages/catalog-core/`, `packages/server-*/` o compila scripts:
...
| Canal | Enlace |
|:--------|:-----|
| 💬 Preguntas | [Abrir una discusión](https://github.com/diegosouzapw/awesome-omni-skills/discussions) |
| 🐛 Errores | [Abrir una incidencia](https://github.com/diegosouzapw/awesome-omni-skills/issues) |
| 📝 Comentarios tempranos | [Abrir un borrador de relaciones públicas](https://github.com/diegosouzapw/awesome-omni-skills/pulls) |