...
>**Si descubres un problema de seguridad en Awesome Omni Skills, no abras un problema público primero.**

Por favor reporte a través de uno de estos canales privados:

| Canal | Cómo |
|:--------|:----|
| 🔒 Aviso de seguridad de GitHub | [Abrir un aviso privado](https://github.com/diegosouzapw/awesome-omni-skills/security/advisories/new) |
...
| Componente | Camino |
|:----------|:-----|
| 🖥️ CLI e instalador | `paquetes/cli/` |
| 📚 Tiempo de ejecución del catálogo compartido | `paquetes/catalog-core/` |
| 🔌 Instalar objetivos | `paquetes/destinos de instalación/` |
| ⚙️ Scripts de compilación y validación | `herramientas/scripts/` |
| 📦 Artefactos de catálogo generados | `dist/` |
| 🌐 Paquetes API, MCP y A2A | `paquetes/servidor-api/` · `paquetes/servidor-mcp/` · `paquetes/servidor-a2a/` |
...
| 🔍 Escáner estático | Inspecciona `SKILL.md`, archivos empaquetados y scripts de ayuda, y bloquea la entrada de hallazgos críticos al catálogo público |
| 🦠 Liberar verificación de malware | Las versiones publicadas se verifican tanto con ClamAV como con VirusTotal antes de su publicación |### 🚫 Hard Content Blocks

El repositorio público no acepta habilidades que salten la puerta de seguridad crítica. Los bloqueadores duros actuales incluyen:

- contenido remoto canalizado directamente a la ejecución del shell
- instrucciones que intentan revelar indicaciones, secretos o contexto de ejecución oculto

Cuando el validador encuentra uno de esos patrones, la habilidad no ingresa al catálogo público. En la admisión del repositorio externo, los hermanos válidos aún pueden continuar mientras las habilidades bloqueadas se excluyen y se informan por separado.### 🖥️ Runtime Controls
...
| 🦠 Puerta de escáner dual | ClamAV y VirusTotal deben completarse limpiamente para la publicación del lanzamiento |
...
Debido a que Awesome Omni Skills incluye ayudas ejecutables, herramientas locales compatibles con el sistema de archivos y escritores de configuraciones específicas del cliente, estas clases de vulnerabilidad deben tratarse como**alta prioridad**incluso si aparecen "solo locales":
