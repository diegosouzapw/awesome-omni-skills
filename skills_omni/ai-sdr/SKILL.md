...
family_id: "ai-sdr"
family_name: "AI SDR Skill"
variant_id: "omni"
variant_label: "Omni Curated"
is_default_variant: "True"
derived_from: "skills/ai-sdr"
...
This public intake copy packages `packages/skills-catalog/skills/(gtm)/ai-sdr` from `https://github.com/tech-leads-club/agent-skills.git` into the native Omni Skills editorial shape without hiding its origin.
...
```text
Use @ai-sdr to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/ai-sdr/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/ai-sdr/scripts/omni_import_list_support_pack.py
```
...
```text
Review @ai-sdr using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
