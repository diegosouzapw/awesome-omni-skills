...
family_id: "architecture-patterns"
family_name: "Architecture Patterns"
variant_id: "omni"
variant_label: "Omni Curated"
is_default_variant: true
derived_from: "skills/architecture-patterns"
upstream_skill: "skills/architecture-patterns"
...
This public intake copy packages `plugins/antigravity-awesome-skills-claude/skills/architecture-patterns` from `https://github.com/sickn33/antigravity-awesome-skills` into the native Omni Skills editorial shape without hiding its origin.
...
```text
Use @architecture-patterns to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @architecture-patterns against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @architecture-patterns for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @architecture-patterns using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/architecture-patterns`, fails to mention provenance, or does not use any copied source files at all.
