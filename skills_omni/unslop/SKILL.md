...
family_id: "unslop"
family_name: "unslop \u2014 Strip AI Writing Patterns via CLI"
variant_id: "omni"
variant_label: "Omni Curated"
is_default_variant: true
derived_from: "skills/unslop"
upstream_skill: "skills/unslop"
upstream_author: "MohamedAbdallah-14"
...
This public intake copy packages `plugins/antigravity-awesome-skills-claude/skills/unslop` from `https://github.com/sickn33/antigravity-awesome-skills` into the native Omni Skills editorial shape without hiding its origin.
...
```text
Use @unslop to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @unslop against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @unslop for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @unslop using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/unslop`, fails to mention provenance, or does not use any copied source files at all.
