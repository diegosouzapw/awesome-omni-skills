...
family_id: "domain-identification-grouping"
family_name: "Domain Identification and Grouping"
variant_id: "omni"
variant_label: "Omni Curated"
is_default_variant: "True"
derived_from: "skills/domain-identification-grouping"
...
This public intake copy packages `packages/skills-catalog/skills/(architecture)/domain-identification-grouping` from `https://github.com/tech-leads-club/agent-skills.git` into the native Omni Skills editorial shape without hiding its origin.
...
```text
Use @domain-identification-grouping to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/domain-identification-grouping/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/domain-identification-grouping/scripts/omni_import_list_support_pack.py
```
...
```text
Review @domain-identification-grouping using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
