...
family_id: "brand-guidelines"
family_name: "Brand Guidelines"
variant_id: "omni"
variant_label: "Omni Curated"
is_default_variant: true
derived_from: "skills/brand-guidelines"
upstream_skill: "skills/brand-guidelines"
...
```text
Use @brand-guidelines to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @brand-guidelines against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @brand-guidelines for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @brand-guidelines using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/brand-guidelines`, fails to mention provenance, or does not use any copied source files at all.
