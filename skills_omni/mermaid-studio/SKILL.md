...
family_id: "mermaid-studio"
family_name: "Mermaid Studio"
variant_id: "omni"
variant_label: "Omni Curated"
is_default_variant: "True"
derived_from: "skills/mermaid-studio"
...
This public intake copy packages `packages/skills-catalog/skills/(tooling)/mermaid-studio` from `https://github.com/tech-leads-club/agent-skills.git` into the native Omni Skills editorial shape without hiding its origin.
...
```text
Use @mermaid-studio to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/mermaid-studio/scripts/omni_import_list_support_pack.py
```
...
```text
Review @mermaid-studio using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
