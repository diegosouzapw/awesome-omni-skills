...
family_id: "security-ownership-map"
family_name: "Security Ownership Map"
variant_id: "omni"
variant_label: "Omni Curated"
is_default_variant: "True"
derived_from: "skills/security-ownership-map"
...
This public intake copy packages `packages/skills-catalog/skills/(security)/security-ownership-map` from `https://github.com/tech-leads-club/agent-skills.git` into the native Omni Skills editorial shape without hiding its origin.
...
```text
Use @security-ownership-map to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/security-ownership-map/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/security-ownership-map/scripts/omni_import_list_support_pack.py
```
...
```text
Review @security-ownership-map using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `packages/skills-catalog/skills/(security)/security-ownership-map`, fails to mention provenance, or does not use the support pack at all.
