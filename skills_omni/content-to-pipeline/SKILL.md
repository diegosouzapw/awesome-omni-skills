...
family_id: "content-to-pipeline"
family_name: "Content-to-Pipeline: Turning Content Into Revenue"
variant_id: "omni"
variant_label: "Omni Curated"
is_default_variant: "True"
derived_from: "skills/content-to-pipeline"
...
This public intake copy packages `packages/skills-catalog/skills/(gtm)/content-to-pipeline` from `https://github.com/tech-leads-club/agent-skills.git` into the native Omni Skills editorial shape without hiding its origin.
...
```text
Use @content-to-pipeline to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/content-to-pipeline/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/content-to-pipeline/scripts/omni_import_list_support_pack.py
```
...
```text
Review @content-to-pipeline using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
