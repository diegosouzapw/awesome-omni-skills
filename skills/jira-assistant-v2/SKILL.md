...
version: "0.0.1"
category: ai-agents
tags: ["jira-assistant-v2", "jira-assistant", "manage", "jira", "issues", "via", "atlassian", "mcp"]
complexity: advanced
risk: caution
...
This public intake copy packages `packages/skills-catalog/skills/(development)/jira-assistant` from `https://github.com/tech-leads-club/agent-skills` into the native Omni Skills editorial shape without hiding its origin.
...
```text
Use @jira-assistant-v2 to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/jira-assistant-v2/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/jira-assistant-v2/scripts/omni_import_list_support_pack.py
```
...
```text
Review @jira-assistant-v2 using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
