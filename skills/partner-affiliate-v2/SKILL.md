...
tags: ["partner-affiliate-v2", "partner-affiliate", "the", "user", "wants", "build", "partner", "program"]
complexity: advanced
risk: safe
...
```text
Use @partner-affiliate-v2 to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/partner-affiliate-v2/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/partner-affiliate-v2/scripts/omni_import_list_support_pack.py
```
...
```text
Review @partner-affiliate-v2 using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
