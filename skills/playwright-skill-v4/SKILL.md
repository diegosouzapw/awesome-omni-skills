...
tags: ["playwright-skill-v4", "playwright-skill", "complete", "browser", "automation", "playwright", "auto-detects", "dev"]
complexity: advanced
risk: caution
...
```text
Use @playwright-skill-v4 to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/playwright-skill-v4/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/playwright-skill-v4/scripts/omni_import_list_support_pack.py
```
...
```text
Review @playwright-skill-v4 using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
