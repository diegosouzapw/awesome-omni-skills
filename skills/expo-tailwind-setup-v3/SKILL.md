...
description: "Tailwind CSS Setup for Expo with react-native-css workflow skill. Use this skill when the user needs Set up Tailwind CSS v4 in Expo with react-native-css and NativeWind v5 for universal styling and the operator should rely on the packaged workflow, support pack, troubleshooting notes, and provenance links before merging or handing off."
version: "0.0.1"
category: frontend
...
This public intake copy packages `plugins/antigravity-bundle-expo-react-native/skills/expo-tailwind-setup` from `https://github.com/sickn33/antigravity-awesome-skills` into the native Omni Skills editorial shape without hiding its origin.
...
```text
Use @expo-tailwind-setup-v3 to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/expo-tailwind-setup-v3/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/expo-tailwind-setup-v3/scripts/omni_import_list_support_pack.py
```
...
```text
Review @expo-tailwind-setup-v3 using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-bundle-expo-react-native/skills/expo-tailwind-setup`, fails to mention provenance, or does not use the support pack at all.
