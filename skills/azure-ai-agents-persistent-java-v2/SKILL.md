...
description: "Azure AI Agents Persistent SDK for Java workflow skill. Use this skill when the user needs Azure AI Agents Persistent SDK for Java. Low-level SDK for creating and managing AI agents with threads, messages, runs, and tools and the operator should rely on the packaged workflow, support pack, troubleshooting notes, and provenance links before merging or handing off."
version: "0.0.1"
category: ai-agents
...
This public intake copy packages `plugins/antigravity-awesome-skills/skills/azure-ai-agents-persistent-java` from `https://github.com/sickn33/antigravity-awesome-skills` into the native Omni Skills editorial shape without hiding its origin.
...
Imported source sections that did not map cleanly to the public headings are still preserved below or in the support files. Notable imported sections: Environment Variables, Authentication, Key Concepts, Error Handling.
...
```text
Use @azure-ai-agents-persistent-java-v2 to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/azure-ai-agents-persistent-java-v2/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/azure-ai-agents-persistent-java-v2/scripts/omni_import_list_support_pack.py
```
...
```text
Review @azure-ai-agents-persistent-java-v2 using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/azure-ai-agents-persistent-java`, fails to mention provenance, or does not use the support pack at all.
