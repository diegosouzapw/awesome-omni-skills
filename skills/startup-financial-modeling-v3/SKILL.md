...
description: "Startup Financial Modeling workflow skill. Use this skill when the user needs Build comprehensive 3-5 year financial models with revenue projections, cost structures, cash flow analysis, and scenario planning for early-stage startups and the operator should rely on the packaged workflow, support pack, troubleshooting notes, and provenance links before merging or handing off."
version: "0.0.1"
category: devops
...
This public intake copy packages `plugins/antigravity-bundle-business-analyst/skills/startup-financial-modeling` from `https://github.com/sickn33/antigravity-awesome-skills` into the native Omni Skills editorial shape without hiding its origin.
...
```text
Use @startup-financial-modeling-v3 to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/startup-financial-modeling-v3/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/startup-financial-modeling-v3/scripts/omni_import_list_support_pack.py
```
...
```text
Review @startup-financial-modeling-v3 using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-bundle-business-analyst/skills/startup-financial-modeling`, fails to mention provenance, or does not use the support pack at all.
