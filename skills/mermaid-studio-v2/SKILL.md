...
version: "0.0.1"
category: devops
tags: ["mermaid-studio-v2", "mermaid-studio", "expert", "mermaid", "diagram", "creation", "validation", "and"]
complexity: advanced
risk: safe
...
This public intake copy packages `packages/skills-catalog/skills/(tooling)/mermaid-studio` from `https://github.com/tech-leads-club/agent-skills` into the native Omni Skills editorial shape without hiding its origin.
...
```text
Use @mermaid-studio-v2 to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/mermaid-studio-v2/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/mermaid-studio-v2/scripts/omni_import_list_support_pack.py
```
...
```text
Review @mermaid-studio-v2 using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `packages/skills-catalog/skills/(tooling)/mermaid-studio`, fails to mention provenance, or does not use the support pack at all.
...
| --- | --- | --- |
| `references` | checklists, rubrics, playbooks, and source summaries | `references/aws-architecture.md` |
| `examples` | prompt packets and usage templates | `examples/omni-import-operator-packet.md` |
| `scripts` | origin inspection and support-pack listing | `scripts/batch.mjs` |
