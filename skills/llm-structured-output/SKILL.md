...
version: "0.0.1"
category: ai-agents
tags: ["llm-structured-output", "ai-agents", "agents", "assets", "examples", "references", "scripts"]
complexity: advanced
risk: caution
...
This public intake copy packages `plugins/antigravity-awesome-skills-claude/skills/llm-structured-output` from `https://github.com/sickn33/antigravity-awesome-skills` into the native Omni Skills editorial shape without hiding its origin.
...
```text
Use @llm-structured-output to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/llm-structured-output/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/llm-structured-output/scripts/omni_import_list_support_pack.py
```
...
```text
Review @llm-structured-output using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/llm-structured-output`, fails to mention provenance, or does not use the support pack at all.
