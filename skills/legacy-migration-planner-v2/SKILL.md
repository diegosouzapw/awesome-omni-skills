---
name: legacy-migration-planner-v2
...
tags: ["legacy-migration-planner-v2", "legacy-migration-planner", "planning", "legacy", "system", "migrations", "codebase", "modernization"]
complexity: advanced
risk: caution
...
```text
Use @legacy-migration-planner-v2 to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/legacy-migration-planner-v2/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/legacy-migration-planner-v2/scripts/omni_import_list_support_pack.py
```
...
```text
Review @legacy-migration-planner-v2 using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
