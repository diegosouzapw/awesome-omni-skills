...
tags: ["positioning-icp-v2", "positioning-icp", "the", "user", "wants", "define", "ideal", "customer"]
complexity: advanced
risk: caution
...
```text
Use @positioning-icp-v2 to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/positioning-icp-v2/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/positioning-icp-v2/scripts/omni_import_list_support_pack.py
```
...
```text
Review @positioning-icp-v2 using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
