---
name: m365-agents-ts
description: "Microsoft 365 Agents SDK (TypeScript) workflow skill. Use this skill when the user needs Microsoft 365 Agents SDK for TypeScript/Node.js and the operator should rely on the packaged workflow, support pack, troubleshooting notes, and provenance links before merging or handing off."
version: "0.0.1"
category: ai-agents
tags: ["m365-agents-ts", "microsoft", "sdk", "for", "typescript", "node", "ai-agents", "agents"]
complexity: advanced
risk: safe
...
This public intake copy packages `plugins/antigravity-awesome-skills-claude/skills/m365-agents-ts` from `https://github.com/sickn33/antigravity-awesome-skills` into the native Omni Skills editorial shape without hiding its origin.
...
Imported source sections that did not map cleanly to the public headings are still preserved below or in the support files. Notable imported sections: Before implementation, Environment Variables, Streaming responses with Azure OpenAI, Copilot Studio client (Direct to Engine), Copilot Studio WebChat integration.
...
```text
Use @m365-agents-ts to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/m365-agents-ts/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/m365-agents-ts/scripts/omni_import_list_support_pack.py
```
...
```text
Review @m365-agents-ts using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/m365-agents-ts`, fails to mention provenance, or does not use the support pack at all.
