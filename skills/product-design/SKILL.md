---
name: product-design
description: "PRODUCT DESIGN \u2014 Nivel Apple workflow skill. Use this skill when the user needs Design de produto nivel Apple \u2014 sistemas visuais, UX flows, acessibilidade, linguagem visual proprietaria, design tokens, prototipagem e handoff. Cobre Figma, design systems, tipografia, cor, espacamento, motion design e principios de design cognitivo and the operator should rely on the packaged workflow, support pack, troubleshooting notes, and provenance links before merging or handing off."
version: "0.0.1"
category: design
...
This public intake copy packages `plugins/antigravity-awesome-skills-claude/skills/product-design` from `https://github.com/sickn33/antigravity-awesome-skills` into the native Omni Skills editorial shape without hiding its origin.
...
```text
Use @product-design to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/product-design/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/product-design/scripts/omni_import_list_support_pack.py
```
...
```text
Review @product-design using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/product-design`, fails to mention provenance, or does not use the support pack at all.
