...
description: "Voice AI Engine Development workflow skill. Use this skill when the user needs Build real-time conversational AI voice engines using async worker pipelines, streaming transcription, LLM agents, and TTS synthesis with interrupt handling and multi-provider support and the operator should rely on the packaged workflow, support pack, troubleshooting notes, and provenance links before merging or handing off."
version: "0.0.1"
category: ai-agents
...
This public intake copy packages `plugins/antigravity-awesome-skills/skills/voice-ai-engine-development` from `https://github.com/sickn33/antigravity-awesome-skills` into the native Omni Skills editorial shape without hiding its origin.
...
```text
Use @voice-ai-engine-development-v2 to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/voice-ai-engine-development-v2/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/voice-ai-engine-development-v2/scripts/omni_import_list_support_pack.py
```
...
```text
Review @voice-ai-engine-development-v2 using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/voice-ai-engine-development`, fails to mention provenance, or does not use the support pack at all.
...
| --- | --- | --- |
| `references` | checklists, rubrics, playbooks, and source summaries | `references/common_pitfalls.md` |
| `examples` | prompt packets and usage templates | `examples/complete_voice_engine.py` |
