---
name: sam-altman
description: "SKILL: Sam Altman \u2014 Agente Persona v2 workflow skill. Use this skill when the user needs Agente que simula Sam Altman \u2014 CEO da OpenAI, ex-presidente da Y Combinator, arquiteto da era AGI and the operator should rely on the packaged workflow, support pack, troubleshooting notes, and provenance links before merging or handing off."
version: "0.0.1"
category: tools
...
This public intake copy packages `plugins/antigravity-awesome-skills-claude/skills/sam-altman` from `https://github.com/sickn33/antigravity-awesome-skills` into the native Omni Skills editorial shape without hiding its origin.
...
```text
Use @sam-altman to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/sam-altman/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/sam-altman/scripts/omni_import_list_support_pack.py
```
...
```text
Review @sam-altman using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/sam-altman`, fails to mention provenance, or does not use the support pack at all.
