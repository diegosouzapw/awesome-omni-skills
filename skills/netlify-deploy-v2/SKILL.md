...
version: "0.0.1"
category: devops
tags: ["netlify-deploy-v2", "netlify-deploy", "deploy", "web", "projects", "netlify", "using", "the"]
complexity: advanced
risk: caution
...
This public intake copy packages `packages/skills-catalog/skills/(cloud)/netlify-deploy` from `https://github.com/tech-leads-club/agent-skills` into the native Omni Skills editorial shape without hiding its origin.
...
```text
Use @netlify-deploy-v2 to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/netlify-deploy-v2/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/netlify-deploy-v2/scripts/omni_import_list_support_pack.py
```
...
```text
Review @netlify-deploy-v2 using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
