...
description: "Senior Fullstack workflow skill. Use this skill when the user needs Complete toolkit for senior fullstack with modern tools and best practices and the operator should rely on the packaged workflow, support pack, troubleshooting notes, and provenance links before merging or handing off."
version: "0.0.1"
...
This public intake copy packages `plugins/antigravity-bundle-full-stack-developer/skills/senior-fullstack` from `https://github.com/sickn33/antigravity-awesome-skills` into the native Omni Skills editorial shape without hiding its origin.
...
```text
Use @senior-fullstack-v3 to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/senior-fullstack-v3/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/senior-fullstack-v3/scripts/omni_import_list_support_pack.py
```
...
```text
Review @senior-fullstack-v3 using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-bundle-full-stack-developer/skills/senior-fullstack`, fails to mention provenance, or does not use the support pack at all.
...
| `scripts` | origin inspection and support-pack listing | `scripts/code_quality_analyzer.py` |
