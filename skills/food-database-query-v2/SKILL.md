...
description: "\u98df\u7269\u6570\u636e\u5e93\u67e5\u8be2\u6280\u80fd workflow skill. Use this skill when the user needs Food Database Query and the operator should rely on the packaged workflow, support pack, troubleshooting notes, and provenance links before merging or handing off."
version: "0.0.1"
category: backend
tags: ["food-database-query-v2", "food-database-query", "food", "database", "query", "backend", "agents", "assets"]
complexity: advanced
risk: safe
...
```text
Use @food-database-query-v2 to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/food-database-query-v2/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/food-database-query-v2/scripts/omni_import_list_support_pack.py
```
...
```text
Review @food-database-query-v2 using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/food-database-query`, fails to mention provenance, or does not use the support pack at all.
