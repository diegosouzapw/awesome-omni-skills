---
name: component-common-domain-detection-v2
...
tags: ["component-common-domain-detection-v2", "component-common-domain-detection", "finds", "duplicate", "business", "logic", "spread", "across"]
complexity: advanced
risk: caution
...
This public intake copy packages `packages/skills-catalog/skills/(architecture)/component-common-domain-detection` from `https://github.com/tech-leads-club/agent-skills` into the native Omni Skills editorial shape without hiding its origin.
...
```text
Use @component-common-domain-detection-v2 to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/component-common-domain-detection-v2/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/component-common-domain-detection-v2/scripts/omni_import_list_support_pack.py
```
...
```text
Review @component-common-domain-detection-v2 using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
