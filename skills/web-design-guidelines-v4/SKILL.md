...
version: "0.0.1"
category: design
tags: ["web-design-guidelines-v4", "web-design-guidelines", "review", "for", "web", "interface", "guidelines", "compliance"]
complexity: advanced
risk: safe
...
```text
Use @web-design-guidelines-v4 to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/web-design-guidelines-v4/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/web-design-guidelines-v4/scripts/omni_import_list_support_pack.py
```
...
```text
Review @web-design-guidelines-v4 using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `packages/skills-catalog/skills/(design)/web-design-guidelines`, fails to mention provenance, or does not use the support pack at all.
...
| --- | --- | --- |
| `references` | checklists, rubrics, playbooks, and source summaries | `references/guideline.md` |
