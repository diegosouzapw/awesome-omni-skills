...
version: "0.0.1"
category: frontend
tags: ["frontend-design", "you", "are", "frontend", "designer-engineer", "not", "layout", "generator"]
complexity: advanced
risk: caution
...
```text
Use @frontend-design to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/frontend-design/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/frontend-design/scripts/omni_import_list_support_pack.py
```
...
```text
Review @frontend-design using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/frontend-design`, fails to mention provenance, or does not use the support pack at all.
