...
description: "modules/vpc/variables.tf workflow skill. Use this skill when the user needs Terraform module creation for AWS \u2014 reusable modules, state management, and HCL best practices. Use when building or reviewing Terraform AWS infrastructure and the operator should rely on the packaged workflow, support pack, troubleshooting notes, and provenance links before merging or handing off."
version: "0.0.1"
category: devops
...
This public intake copy packages `plugins/antigravity-awesome-skills/skills/terraform-aws-modules` from `https://github.com/sickn33/antigravity-awesome-skills` into the native Omni Skills editorial shape without hiding its origin.
...
```text
Use @terraform-aws-modules-v2 to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/terraform-aws-modules-v2/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/terraform-aws-modules-v2/scripts/omni_import_list_support_pack.py
```
...
```text
Review @terraform-aws-modules-v2 using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/terraform-aws-modules`, fails to mention provenance, or does not use the support pack at all.
