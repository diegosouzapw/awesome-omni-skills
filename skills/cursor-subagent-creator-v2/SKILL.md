---
name: cursor-subagent-creator-v2
...
version: "0.0.1"
category: ai-agents
tags: ["cursor-subagent-creator-v2", "cursor-subagent-creator", "creates", "cursor-specific", "subagents", "isolated", "context", "for"]
complexity: advanced
risk: caution
...
This public intake copy packages `packages/skills-catalog/skills/(creation)/cursor-subagent-creator` from `https://github.com/tech-leads-club/agent-skills` into the native Omni Skills editorial shape without hiding its origin.
...
```text
Use @cursor-subagent-creator-v2 to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/cursor-subagent-creator-v2/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/cursor-subagent-creator-v2/scripts/omni_import_list_support_pack.py
```
...
```text
Review @cursor-subagent-creator-v2 using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
