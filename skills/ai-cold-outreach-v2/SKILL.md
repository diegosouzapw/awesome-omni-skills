...
tags: ["ai-cold-outreach-v2", "ai-cold-outreach", "the", "user", "wants", "build", "ai-powered", "outreach"]
complexity: advanced
risk: caution
...
```text
Use @ai-cold-outreach-v2 to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/ai-cold-outreach-v2/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/ai-cold-outreach-v2/scripts/omni_import_list_support_pack.py
```
...
```text
Review @ai-cold-outreach-v2 using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
