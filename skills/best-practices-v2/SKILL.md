...
tags: ["best-practices-v2", "best-practices", "apply", "modern", "web", "development", "best", "practices"]
complexity: advanced
risk: caution
...
```text
Use @best-practices-v2 to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/best-practices-v2/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/best-practices-v2/scripts/omni_import_list_support_pack.py
```
...
```text
Review @best-practices-v2 using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
