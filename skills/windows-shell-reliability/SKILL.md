...
description: "Windows Shell Reliability Patterns workflow skill. Use this skill when the user needs Reliable command execution on Windows: paths, encoding, and common binary pitfalls and the operator should rely on the packaged workflow, support pack, troubleshooting notes, and provenance links before merging or handing off."
version: "0.0.1"
...
This public intake copy packages `plugins/antigravity-awesome-skills-claude/skills/windows-shell-reliability` from `https://github.com/sickn33/antigravity-awesome-skills` into the native Omni Skills editorial shape without hiding its origin.
...
```text
Use @windows-shell-reliability to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/windows-shell-reliability/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/windows-shell-reliability/scripts/omni_import_list_support_pack.py
```
...
```text
Review @windows-shell-reliability using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/windows-shell-reliability`, fails to mention provenance, or does not use the support pack at all.
