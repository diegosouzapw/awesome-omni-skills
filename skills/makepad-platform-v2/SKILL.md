...
version: "0.0.1"
category: frontend
tags: ["makepad-platform-v2", "makepad-platform", "frontend", "agents", "assets", "examples", "references", "scripts"]
complexity: advanced
risk: safe
...
This public intake copy packages `plugins/antigravity-awesome-skills/skills/makepad-platform` from `https://github.com/sickn33/antigravity-awesome-skills` into the native Omni Skills editorial shape without hiding its origin.
...
```text
Use @makepad-platform-v2 to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/makepad-platform-v2/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/makepad-platform-v2/scripts/omni_import_list_support_pack.py
```
...
```text
Review @makepad-platform-v2 using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/makepad-platform`, fails to mention provenance, or does not use the support pack at all.
