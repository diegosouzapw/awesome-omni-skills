...
This public intake copy packages `plugins/antigravity-bundle-essentials/skills/kaizen` from `https://github.com/sickn33/antigravity-awesome-skills` into the native Omni Skills editorial shape without hiding its origin.
...
```text
Use @kaizen-v3 to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/kaizen-v3/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/kaizen-v3/scripts/omni_import_list_support_pack.py
```
...
```text
Review @kaizen-v3 using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-bundle-essentials/skills/kaizen`, fails to mention provenance, or does not use the support pack at all.
