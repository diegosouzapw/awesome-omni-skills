...
tags: ["multi-platform-launch-v2", "multi-platform-launch", "the", "user", "wants", "launch", "product", "across"]
complexity: advanced
risk: safe
...
This public intake copy packages `packages/skills-catalog/skills/(gtm)/multi-platform-launch` from `https://github.com/tech-leads-club/agent-skills` into the native Omni Skills editorial shape without hiding its origin.
...
```text
Use @multi-platform-launch-v2 to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/multi-platform-launch-v2/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/multi-platform-launch-v2/scripts/omni_import_list_support_pack.py
```
...
```text
Review @multi-platform-launch-v2 using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
