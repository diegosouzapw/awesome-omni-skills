...
Imported source sections that did not map cleanly to the public headings are still preserved below or in the support files. Notable imported sections: Environment Configuration, Voice Options, Realtime API Events, Audio Format.
...
```text
Use @podcast-generation to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/podcast-generation/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/podcast-generation/scripts/omni_import_list_support_pack.py
```
...
```text
Review @podcast-generation using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/podcast-generation`, fails to mention provenance, or does not use the support pack at all.
