...
description: "SKILL DE EDITAL \u2014 AN\u00c1LISE PERICIAL DE EDITAIS DE LEIL\u00c3O workflow skill. Use this skill when the user needs Analise e auditoria de editais de leilao judicial e extrajudicial. Riscos ocultos, clausulas perigosas, debitos, ocupante e classificacao da oportunidade and the operator should rely on the packaged workflow, support pack, troubleshooting notes, and provenance links before merging or handing off."
version: "0.0.1"
category: devops
...
This public intake copy packages `plugins/antigravity-awesome-skills/skills/leiloeiro-edital` from `https://github.com/sickn33/antigravity-awesome-skills` into the native Omni Skills editorial shape without hiding its origin.
...
```text
Use @leiloeiro-edital-v2 to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/leiloeiro-edital-v2/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/leiloeiro-edital-v2/scripts/omni_import_list_support_pack.py
```
...
```text
Review @leiloeiro-edital-v2 using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/leiloeiro-edital`, fails to mention provenance, or does not use the support pack at all.
