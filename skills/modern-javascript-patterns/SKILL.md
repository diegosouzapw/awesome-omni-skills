...
tags: ["modern-javascript-patterns", "comprehensive", "guide", "for", "mastering", "modern", "javascript", "es6"]
complexity: beginner
risk: safe
...
This public intake copy packages `plugins/antigravity-awesome-skills-claude/skills/modern-javascript-patterns` from `https://github.com/sickn33/antigravity-awesome-skills` into the native Omni Skills editorial shape without hiding its origin.
...
```text
Use @modern-javascript-patterns to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @modern-javascript-patterns against EXTERNAL_SOURCE.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @modern-javascript-patterns for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @modern-javascript-patterns using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/modern-javascript-patterns`, fails to mention provenance, or does not use any copied source files at all.
