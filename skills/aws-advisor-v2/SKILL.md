...
version: "0.0.1"
category: devops
tags: ["aws-advisor-v2", "aws-advisor", "expert", "aws", "cloud", "advisor", "for", "architecture"]
complexity: advanced
risk: safe
...
```text
Use @aws-advisor-v2 to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/aws-advisor-v2/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/aws-advisor-v2/scripts/omni_import_list_support_pack.py
```
...
```text
Review @aws-advisor-v2 using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
