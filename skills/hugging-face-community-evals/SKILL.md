...
This public intake copy packages `plugins/antigravity-awesome-skills-claude/skills/hugging-face-community-evals` from `https://github.com/sickn33/antigravity-awesome-skills` into the native Omni Skills editorial shape without hiding its origin.
...
Imported source sections that did not map cleanly to the public headings are still preserved below or in the support files. Notable imported sections: Option A: inspect-ai with local inference providers path, Option B: inspect-ai on Local GPU, Option C: lighteval on Local GPU.
...
```text
Use @hugging-face-community-evals to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/hugging-face-community-evals/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/hugging-face-community-evals/scripts/omni_import_list_support_pack.py
```
...
```text
Review @hugging-face-community-evals using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/hugging-face-community-evals`, fails to mention provenance, or does not use the support pack at all.
