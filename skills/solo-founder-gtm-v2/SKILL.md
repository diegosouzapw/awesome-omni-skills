...
tags: ["solo-founder-gtm-v2", "solo-founder-gtm", "the", "user", "solo", "founder", "building", "gtm"]
complexity: advanced
risk: caution
...
```text
Use @solo-founder-gtm-v2 to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/solo-founder-gtm-v2/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/solo-founder-gtm-v2/scripts/omni_import_list_support_pack.py
```
...
```text
Review @solo-founder-gtm-v2 using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
