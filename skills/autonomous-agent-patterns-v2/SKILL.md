...
description: "\ud83d\udd79\ufe0f Autonomous Agent Patterns workflow skill. Use this skill when the user needs Design patterns for building autonomous coding agents, inspired by Cline and OpenAI Codex and the operator should rely on the packaged workflow, support pack, troubleshooting notes, and provenance links before merging or handing off."
version: "0.0.1"
category: ai-agents
...
This public intake copy packages `plugins/antigravity-awesome-skills/skills/autonomous-agent-patterns` from `https://github.com/sickn33/antigravity-awesome-skills` into the native Omni Skills editorial shape without hiding its origin.
...
```text
Use @autonomous-agent-patterns-v2 to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/autonomous-agent-patterns-v2/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/autonomous-agent-patterns-v2/scripts/omni_import_list_support_pack.py
```
...
```text
Review @autonomous-agent-patterns-v2 using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/autonomous-agent-patterns`, fails to mention provenance, or does not use the support pack at all.
