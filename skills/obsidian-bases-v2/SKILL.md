...
description: "Obsidian Bases Skill workflow skill. Use this skill when the user needs Create and edit Obsidian Bases (.base files) with views, filters, formulas, and summaries. Use when working with .base files, creating database-like views of notes, or when the user mentions Bases, table views, card views, filters, or formulas in Obsidian and the operator should rely on the packaged workflow, support pack, troubleshooting notes, and provenance links before merging or handing off."
version: "0.0.1"
category: backend
...
```text
Use @obsidian-bases-v2 to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/obsidian-bases-v2/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/obsidian-bases-v2/scripts/omni_import_list_support_pack.py
```
...
```text
Review @obsidian-bases-v2 using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/obsidian-bases`, fails to mention provenance, or does not use the support pack at all.
...
| --- | --- | --- |
| `references` | checklists, rubrics, playbooks, and source summaries | `references/FUNCTIONS_REFERENCE.md` |
