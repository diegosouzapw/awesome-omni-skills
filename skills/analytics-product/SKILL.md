...
description: "ANALYTICS-PRODUCT \u2014 Decida com Dados workflow skill. Use this skill when the user needs Analytics de produto \u2014 PostHog, Mixpanel, eventos, funnels, cohorts, retencao, north star metric, OKRs e dashboards de produto and the operator should rely on the packaged workflow, support pack, troubleshooting notes, and provenance links before merging or handing off."
version: "0.0.1"
category: data-ai
...
This public intake copy packages `plugins/antigravity-awesome-skills-claude/skills/analytics-product` from `https://github.com/sickn33/antigravity-awesome-skills` into the native Omni Skills editorial shape without hiding its origin.
...
```text
Use @analytics-product to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```bash
python3 skills/analytics-product/scripts/omni_import_print_origin.py
```
...
```bash
python3 skills/analytics-product/scripts/omni_import_list_support_pack.py
```
...
```text
Review @analytics-product using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/analytics-product`, fails to mention provenance, or does not use the support pack at all.
